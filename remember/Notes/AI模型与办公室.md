...
- 无本地函数工具的聊天请求启用方舟 `web_search`（`max_keyword=3`）；2026-07-15 已用同一 API Key 实测返回 `web_search_call` 和实时结果。本地行情/新闻工具不受影响。若未来服务权限异常，适配器会自动停用搜索并注入“不得编造实时信息”约束。请求统一 `store=false`，减少持仓和交易上下文在供应商侧的留存。
