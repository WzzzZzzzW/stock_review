...
from services.copilot_service import build_copilot_context, enforce_evidence_boundary
...
def chat_stream(body: CopilotChatIn):
    ...
    if chat_id:
        ...
        title = f"{target}：{question[:20]}" if target else question[:30]
    ...
    def _stream():
        try:
            context = build_copilot_context(body.context)
            final_response = ""
            final_tools: list[dict] = []
            for event in office_service.chat_with_agent_stream(
                "copilot", question, history, context
            ):
                ...
                yield f"data: {json.dumps({**event, 'chat_id': chat_id}, ensure_ascii=False)}\n\n"
            ...
            office_db.add_message(chat_id, "assistant", saved, agent_id="copilot")
            yield 'data: "[DONE]"\n\n'
