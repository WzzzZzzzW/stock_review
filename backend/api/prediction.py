...
def _score_stock_v2(
    ...
    if pct >= 9.5:
        mom = 1.0
        reasons.append(
            f"价格动量：今日涨停（+{pct:.2f}%）。这只证明当日动量极强，"
            f"不能单独推导明日方向，必须结合封单、量能、板块和公告因子裁决")
    elif pct >= 5:
        mom = min(1.0, pct / 8)
        reasons.append(
            f"价格动量：今日大涨+{pct:.2f}%，动量为正但获利盘同步增加；"
            f"最终结论服从量能、均线、板块和事件因子的合成分")
    elif pct >= 2:
        mom = pct / 6
        reasons.append(
            f"价格动量：今日上涨+{pct:.2f}%，只计为一项正证据，不直接预测明日上涨")
    elif pct >= 0.3:
        mom = pct / 8
        reasons.append(
        ...
        mom = pct / 6
        reasons.append(
            f"价格动量：今日下跌{pct:.2f}%，短线动量为负；是否回避由量价、趋势和事件因子共同决定")
    elif pct <= -9.5:
        mom = -1.0
        reasons.append(
            f"价格动量：今日跌停（{pct:.2f}%），属于极端负动量；仍需由事件、流动性和趋势因子确认风险等级")
    else:
        mom = max(-1.0, pct / 8)
        reasons.append(
            f"价格动量：今日大跌{pct:.2f}%，负动量显著；不以该数字单独生成买卖指令")
